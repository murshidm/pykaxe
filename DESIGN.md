...
- **Borders are the only structural device.** No filled panels, no drop
  ...
  focused/active.
- **Two accent colors, used consistently, nothing else.** Amber (`TOOL`)
  always means "this identifies a tool." Green (`RESULT`) always means
  "this is output the running subprocess produced," as opposed to pykaxe's
  own chrome (`MUTED`/`WHITE`). No other color is used for meaning.
...
Defined as module-level constants in `app.py` (lines 30–35) and interpolated
...
| --- | --- | --- | --- |
| `WHITE` | `#f2f2f2` | primary/focused | body text, focused input/log border, badge text-on-amber is `#1a1a1a` instead |
| `MUTED` | `#8a8a8a` | secondary/inactive | help text, prompts, hint lines, unfocused hover states |
...
| `TOOL` | `#f2c94c` (amber) | tool identity | `/toolname` in welcome list & suggestions, the badge background, tool banner |
| `RESULT` | `#7ee787` (green) | live subprocess output | every line streamed from a running tool's stdout |
...
- **Style:** solid amber (`TOOL`) background, dark text (`#1a1a1a`, hardcoded
  — the one color not drawn from the shared palette, chosen for contrast
  against amber rather than reusing `WHITE`), bold, left-aligned, 1 row tall,
  `0 1` padding.
...
- **Scrollbar:** fully transparent at rest (`scrollbar-background/-color:
  transparent`), becomes `{MUTED}` on hover and `{WHITE}` while actively
...
- **Text color semantics inside this pane** (not CSS, but content markup
  applied per-line in Python):
  - default/unstyled → terminal foreground
  - `{MUTED}` → pykaxe's own chrome: prompts, hints, status messages
    ("cancelled", "interrupted", "exited with code…")
  - `{TOOL}` → tool names (welcome list, banners)
  - `{RESULT}` (green) → lines streamed live from a running tool's stdout —
    this is the one color that distinguishes "the subprocess said this"
    from "pykaxe said this"
  - `{WHITE}` → the one-off `pykaxe v0.1.x` title line
...
- **Shown state:** up to 8 fuzzy-matched tool names, each rendered as
  `[TOOL]toolname[/]: description` — same amber-for-name convention as the
...
- **Border:** `round {BORDER}` at rest → `round {WHITE}` on `:focus`. This
...
- **Buttons:** `[esc] interrupt`, `[ctrl+y] copy`, `[ctrl+s] scan`,
  ...
  `[WHITE]<key>[/] [MUTED]<label>[/]`, i.e. the keybinding is bright, the
  action name is dim. This is the one place `WHITE` is used for something
  other than "focused."
...
- **Hover:** border brightens to `{WHITE}`, background stays `{BG}` (no
  fill-on-hover).
...
- **Panel (`#picker`):** `width: 80%; height: 80%`, `background: {BG}`
  ...
  `border: round {WHITE}` — **unconditional** white, unlike every other
  ...
  the *same* `WHITE` used elsewhere for focus, but applied statically here
...
| --- | --- | --- | --- |
| Input | border `BORDER` | border `WHITE` | — |
| RichLog | border `BORDER` | (no change — not typically focused) | — |
| Buttons | border `BORDER`, bg `BG` | border unchanged, `text-style: none` forced | border `WHITE` |
| Scrollbars (RichLog/#suggestions) | transparent | — | thumb `MUTED`, track transparent |
| Scrollbars (actively scrolling) | — | thumb `WHITE` | — |
...
- **Recolor anything:** edit the constants at the top of `app.py` (`WHITE`,
  `MUTED`, `BORDER`, `BG`, `TOOL`, `RESULT`) — every rule below references
  these, nothing is hardcoded elsewhere except the badge's `#1a1a1a` text
  and the file picker's unconditional `WHITE` border.
